...
public:
    ...
    virtual void operator()(const Range &range) const CV_OVERRIDE
    {
        for (int r = range.start; r < range.end; r++)
        {
            int row = r / m_width;
            int col = r % m_width;
            cv::Vec3b pixel = m_in.at<cv::Vec3b>(row, col);
                ...
                for (int m = -kSize; m < kSize + 1; m++)
                {
                        ...
                        col + m >= 0 && col + m < m_width)
                    {
                        float mat_value = m_in[(row + k) * m_width +
                                               col + m];
                        float kernel_value = m_kernel[(k + kSize) *
                                                          orig_kernel_size +
                                                      m + kSize];
...
{
public:
    ParallelSobel(image_t *matrix_in, image_t *magnitude, float *theta,
    ...
    virtual void operator()(const Range &range) const CV_OVERRIDE
    {
        ...
        for (int r = range.start; r < range.end; r++)
        {
            ...
            for (int k = -kernelSize; k < kernelSize + 1; k++)
            {
                for (int m = -kernelSize; m < kernelSize + 1; m++)
                {
                    if (row + k >= 0 && row + k < m_height &&
                        col + m >= 0 && col + m < m_width)
                    {
                        float mat_value = m_in[(row + k) * m_width + col + m];
            ...
            m_mag[row * m_width + col] = hypotenuse;
            if (hypotenuse > max)
            {
                max = hypotenuse;
            }
            m_theta[row * m_width + col] = atan2(Ty, Tx);
        }
...
class ParallelSuppression : public ParallelLoopBody
{
public:
    ParallelSuppression(image_t *image_in, image_t *output, float *theta,
                        const int height, const int width, float max_in,
                        float *max_out)
        : m_in(image_in), m_out(output), m_theta(theta), m_height(height),
    ...
    virtual void operator()(const Range &range) const CV_OVERRIDE
    {
        ...
        for (int a = range.start; a < range.end; a++)
        {
            ...
            int q = 255;
            int r = 255;
            float angle = m_theta[i * m_width + k] * 180 / M_PI;
            if (angle < 0)
            {
                angle += 180;
            }
    ...
    ParallelSuppression &operator=(const ParallelSuppression &)
    {
        return *this;
    };
...
public:
    ...
    virtual void operator()(const Range &range) const CV_OVERRIDE
    {
        float thresh_high = m_max * m_high;
        float thresh_low = m_high * m_low;
        int kernelSize = SOBEL_SIZE / 2;
        float max = 0;
        for (int r = range.start; r < range.end; r++)
        {
            ...
            if (m_in[matrix_ind] >= thresh_high)
            {
                m_out[matrix_ind] = STRONG;
            }
            else if (m_in[matrix_ind] >= thresh_low)
            {
