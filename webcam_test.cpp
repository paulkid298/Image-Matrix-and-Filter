...
int process_camera()
{
    ...
    while (video.read(frame))
    {
        ...
        //  covert to grayscale
        grayscale(frame, *matrix1, image_width, image_height);
        // apply gaussian blur
...
void sobel_filter(image_t *matrix_in, image_t *magnitude, float *theta,
...
image_t non_max_suppression(const image_t *matrix_in, image_t *matrix_out, float *theta,
                            int height, int width)
{
    image_t max = 0;
    for (int i = 0; i < height; i++)
    {
        for (int k = 0; k < width; k++)
        {
            ...
            {
                q = (k + 1 < width) ? matrix_in[i * width + k + 1] : 255;
                r = (k - 1 >= 0) ? matrix_in[i * width + k - 1] : 255;
            }
            else if (angle >= 22.5 && angle < 67.5)
            {
                q = (i + 1 < height && k - 1 >= 0) ? matrix_in[(i + 1) * width + k - 1] : 255;
                r = (i - 1 >= 0 && k + 1 < width) ? matrix_in[(i - 1) * width + k + 1] : 255;
            }
            else if (angle >= 67.5 && angle < 112.5)
            {
                q = (i + 1 < height) ? matrix_in[(i + 1) * width + k] : 255;
                r = (i - 1 >= 0) ? matrix_in[(i - 1) * width + k] : 255;
            }
            else if (angle >= 112.5 && angle < 157.5)
            {
                q = (i - 1 >= 0 && k - 1 >= 0) ? matrix_in[(i - 1) * width + k - 1] : 255;
                r = (i + 1 < height && k + 1 < width) ? matrix_in[(i + 1) * width + k + 1] : 255;
            }

            image_t image_val = matrix_in[i * width + k];
...
void hysteresis(image_t *matrix, int height, int width)
{
    for (int i = 0; i < height; i++)
    {
        for (int k = 0; k < width; k++)
            ...
            if (matrix[i * width + k] == WEAK)
            {
                ...
                for (int a = -1; a <= 1; a++)
                {
                    for (int b = -1; b <= 1; b++)
                    {
                        ...
                        {
                            if (matrix[(i + a) * width + (k + b)] == STRONG)
                            {
                                matrix[i * width + k] = STRONG;
                                break;
                            }
                        }
                    }
                    // break if the image
                    if (matrix[i * width + k] == STRONG)
                    {
                        break;
                    }
                }
